@echo off
echo Compiling Psylocke's character assets 
...
set pcPkgb=psylocke_15401.pkgb.json
set pcPkgbNc=psylocke_15401_nc.pkgb.json
...
if  %consoleChoice%==PC (
	del >nul "0. Staging\3. No Cel Shade Assets\actors\15409.igb"
	del >nul "0. Staging\actors\15409.igb"
	del >nul "0. Staging\hud\hud_head_15409.igb"
	del >nul "0. Staging\ui\hud\characters\15409.igb"
)
REM bonus files that won't work on consoles
if  %consoleChoice%==PSP (
	rmdir /s /q "0. Staging\Psylocke - Custom X-Treme Gear Conversation Portrait"
	rmdir /s /q "0. Staging\Psylocke - XML1 Prototype Boltons"
)
